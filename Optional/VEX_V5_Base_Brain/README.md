...
This folder contains a VEXcode V5 Python template that keeps manual controller drive on the V5 Brain while streaming base motion telemetry to the Raspberry Pi over the Brain's USB console serial port.
...
```json
{"x.vel": 0.12, "y.vel": -0.03, "theta.vel": 18.0}
```
...
duration of the replay. When replay ends, the Pi restores the normal slot-8
