...
class LeKiwi(Robot):
    ...
    def __init__(self, config: LeKiwiConfig):
        ...
        self.calibration = {name: cal for name, cal in self.calibration.items() if name in motors}
    ...
    def calibrate(self) -> None:
        ...
        for name in self.arm_motors:
            self.bus.write("Operating_Mode", name, OperatingMode.POSITION.value, num_retry=10)
    ...
    def configure(self):
        ...
        self.bus.disable_torque(num_retry=10)
        self.bus.configure_motors()
        for name in self.arm_motors:
            self.bus.write("Operating_Mode", name, OperatingMode.POSITION.value, num_retry=10)
            # Set P_Coefficient to lower value to avoid shakiness (Default is 32)
            self.bus.write("P_Coefficient", name, 16, num_retry=10)
            # Set I_Coefficient and D_Coefficient to default value 0 and 32
            self.bus.write("I_Coefficient", name, 0, num_retry=10)
            self.bus.write("D_Coefficient", name, 32, num_retry=10)

        for name in self.base_motors:
            self.bus.write("Operating_Mode", name, OperatingMode.VELOCITY.value, num_retry=10)

        self.bus.enable_torque(num_retry=10)
    ...
    def get_observation(self) -> RobotObservation:
        ...
        arm_pos = self.bus.sync_read("Present_Position", self.arm_motors, num_retry=10)
        base_wheel_vel = self.bus.sync_read("Present_Velocity", self.base_motors, num_retry=10)
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        arm_goal_pos = {
            ...
            if k.endswith(".pos") and k.removesuffix(".pos") in self.arm_motors
        }
        ...
        if self.config.max_relative_target is not None:
            present_pos = self.bus.sync_read("Present_Position", self.arm_motors, num_retry=10)
        ...
        return {**{f"{motor}.pos": val for motor, val in arm_goal_pos.items()}, **base_goal_vel}
